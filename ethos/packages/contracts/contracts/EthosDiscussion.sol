...
contract EthosDiscussion is ITargetStatus, Common, AccessControl, UUPSUpgradeable {
    ...
    mapping(address => mapping(uint256 => uint256[])) private directReplyIdsByTargetAddressAndTargetId; // targetContract => taget id => reply ids. it can be 1) contractWithOriginalComment => comment id => reply ids; 2) EthosDiscussion => reply id => reply ids
     ...
     */
    function addReply(address targetContract, uint256 targetId, string memory content, string memory metadata)
        external
        onlyNonZeroAddress(targetContract)
        whenNotPaused
    {
        uint256 authorID = IEthosProfile(contractAddressManager.getContractAddressForName(ETHOS_PROFILE))
     ...
     */
    function editReply(uint256 replyId, string memory content, string memory metadata) external whenNotPaused {
        uint256 authorID = IEthosProfile(contractAddressManager.getContractAddressForName(ETHOS_PROFILE))
            .verifiedProfileIdForAddress(msg.sender);
     ...
     * @return result Replies.
     */
    function repliesByAuthorInRange(uint256 author, uint256 fromIdx, uint256 maxLength)
        external
        view
        returns (Reply[] memory result)
    {
     ...
     * @return result Replies.
     */
    function directRepliesInRange(address targetContract, uint256 parentId, uint256 fromIdx, uint256 maxLength)
        external
        view
        returns (Reply[] memory result)
    {
     ...
     */
    function directReplyCount(address targetContract, uint256 targetId) external view returns (uint256 count) {
