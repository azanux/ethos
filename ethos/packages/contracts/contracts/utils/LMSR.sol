...
import {UD60x18, unwrap, convert, exp, floor} from "@prb/math/src/UD60x18.sol";
...
library LMSR {
     ...
     */
    function _cost(uint256 yesVotes, uint256 noVotes, uint256 liquidityParameter)
        public
        pure
        returns (uint256 costResult)
    {
     ...
     */
    function _getExponentials(uint256 yesVotes, uint256 noVotes, uint256 liquidityParameter)
        public
        pure
        returns (UD60x18 yesExp, UD60x18 noExp)
    {
        // maxSafeRatio chosen as 133 since exp input ~133.084 causes overflow
        uint256 maxSafeRatio = 133;

        if (yesVotes > maxSafeRatio * liquidityParameter || noVotes > maxSafeRatio * liquidityParameter) {
            revert VotesExceedSafeLimit(
                yesVotes > noVotes ? yesVotes : noVotes, liquidityParameter, maxSafeRatio * liquidityParameter
            );
        }
