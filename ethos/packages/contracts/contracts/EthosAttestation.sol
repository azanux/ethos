...
import {
    ...
    AttestationNotFound,
    AttestationInvalid
} from "./errors/AttestationErrors.sol";
...
contract EthosAttestation is IEthosAttestation, AccessControl, UUPSUpgradeable {
    ...
    event AttestationCreated(
        uint256 indexed profileId, string service, string account, string evidence, uint256 indexed attestationId
    );
    ...
    event AttestationClaimed(
        uint256 indexed attestationId, string service, string account, string evidence, uint256 indexed profileId
    );
    ...
    function createAttestation(
        ...
        validateAndSaveSignature(
            _keccakForCreateAttestation(
                profileId, randValue, attestationDetails.account, attestationDetails.service, evidence
            ),
            signature
        );
        ...
        hashIndexByProfileIdAndHash[profileId][hashStr] = attestationHashesByProfileId[profileId].length - 1;
        ...
        emit AttestationCreated(
            profileId, attestationDetails.service, attestationDetails.account, evidence, attestationCount
        );
        attestationCount++;
    }
     ...
     */
    function _claimAttestation(uint256 profileId, bytes32 attestationHash, string calldata evidence)
        private
        returns (bool)
    {
        ...
        (bool profileExists, bool isArchived, bool isMock) = IEthosProfile(ethosProfile).profileStatusById(profileId);
        ...
        bool senderBelongsToProfile = IEthosProfile(ethosProfile).addressBelongsToProfile(msg.sender, profileId);
        ...
        hashIndexByProfileIdAndHash[profileId][attestationHash] = attestationHashesByProfileId[profileId].length - 1;
     ...
     */
    function getAttestationByHash(bytes32 _hash) external view returns (Attestation memory attestation) {
     ...
     */
    function getAttestationIndexByProfileIdAndHash(uint256 profileId, bytes32 attestationHash)
        public
        view
        returns (uint256)
    {
     ...
     */
    function _removeAttestationFromPreviousProfile(uint256 profileId, bytes32 attestationHash) private {
        bytes32[] storage hashes = attestationHashesByProfileId[profileId];
