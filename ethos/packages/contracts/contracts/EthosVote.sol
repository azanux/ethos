...
contract EthosVote is AccessControl, Common, UUPSUpgradeable {
    ...
    event Voted(
        bool isUpvote, uint256 indexed voter, address indexed targetContract, uint256 indexed targetId, uint256 voteId
    );
     ...
     */
    function _modifyVote(address targetContract, uint256 targetId, uint256 voter, bool isUpvote) internal {
     ...
     */
    function voteIndexFor(uint256 voter, address targetContract, uint256 targetId) public view returns (uint256) {
        VotesGeneral storage vg = votesGeneralByContractByTargetId[targetContract][targetId];
     ...
     */
    function hasVotedFor(uint256 voter, address targetContract, uint256 targetId) public view returns (bool) {
        uint256 voteIndex = voteIndexFor(voter, targetContract, targetId);
        return voteIndex != 0;
    }
     ...
     */
    function votesCountFor(address targetContract, uint256 targetId)
        external
        view
     ...
     */
    function votesInRangeFor(address targetContract, uint256 targetId, uint256 fromIdx, uint256 maxLength)
        external
        view
     ...
     * @param isUpvote Whether upvote or downvote.
     */
    function _recordVote(uint256 voter, address targetContract, uint256 targetId, bool isUpvote) private {
        VotesGeneral storage vg = votesGeneralByContractByTargetId[targetContract][targetId];
     ...
     */
    function _votesInRange(uint256 maxLength, uint256 fromIdx, uint256[] memory idArray)
        private
        view
        returns (Vote[] memory result)
    {
