...
contract EthosVouch is AccessControl, UUPSUpgradeable, ITargetStatus, ReentrancyGuard {
    ...
    function vouchByAddress(address subjectAddress, string calldata comment, string calldata metadata)
        ...
        whenNotPaused
    {
        IEthosProfile profile = IEthosProfile(contractAddressManager.getContractAddressForName(ETHOS_PROFILE));
    ...
    function vouchByProfileId(uint256 subjectProfileId, string calldata comment, string calldata metadata)
        ...
        nonReentrant
    {
        uint256 authorProfileId = IEthosProfile(contractAddressManager.getContractAddressForName(ETHOS_PROFILE))
        ...
        if (vouchIdsForSubjectProfileId[subjectProfileId].length >= maximumVouches) {
            revert MaximumVouchesExceeded(
                vouchIdsForSubjectProfileId[subjectProfileId].length, "Exceeds subject vouch limit"
            );
        }
        ...
        vouchIdsForSubjectProfileIdIndex[subjectProfileId][count] =
            vouchIdsForSubjectProfileId[subjectProfileId].length - 1;
    ...
    function markUnhealthy(uint256 vouchId) public whenNotPaused {
        Vouch storage v = vouches[vouchId];
        uint256 profileId = IEthosProfile(contractAddressManager.getContractAddressForName(ETHOS_PROFILE))
            .verifiedProfileIdForAddress(msg.sender);
     ...
     */
    function _rewardPreviousVouchers(uint256 amount, uint256 subjectProfileId, uint256 excludeAuthorId)
        internal
        returns (uint256 amountDistributed)
    {
    ...
    function vouchExistsFor(uint256 author, uint256 subjectProfileId) public view returns (bool) {
        ...
        return v.authorProfileId == author && v.subjectProfileId == subjectProfileId
     ...
     */
    function applyFees(uint256 amount, bool isEntry, uint256 subjectProfileId, uint256 authorProfileId)
        internal
        returns (uint256 toDeposit, uint256 totalFees)
    {
        if (isEntry) {
            // Sum all entry fees to calculate a max fee to be split across each fee type
            uint256 totalFeesBasisPoints =
            ...
            totalFees = amount
                - (amount.mulDiv(BASIS_POINT_SCALE, (BASIS_POINT_SCALE + totalFeesBasisPoints), Math.Rounding.Floor));
            ...
            uint256 protocolFee =
                totalFees.mulDiv(entryProtocolFeeBasisPoints, totalFeesBasisPoints, Math.Rounding.Floor);

            // Donation fee share = (donationFeeBasisPoints / totalBasisPoints) * totalFees
            uint256 donationFee =
                totalFees.mulDiv(entryDonationFeeBasisPoints, totalFeesBasisPoints, Math.Rounding.Floor);
            ...
            if (vouchersPoolFee > 0) {
                vouchersPoolFee = _rewardPreviousVouchers(vouchersPoolFee, subjectProfileId, authorProfileId);
            }
